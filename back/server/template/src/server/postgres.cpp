...
*/
result addUsertoRoom(pqxx::work tx, string username, string password){//create user
	return tx.exec("INSERT INTO users VALUES (" + username + "," + password + ");");
}

bool userExists(pqxx::word tx, string username){//check if user exists
	return tx.exec("FROM users; WHERE [username] LIKE '" + username + "'").size();
}

result createUser(pqxx::word tx, string username, string password){//if username not used create it with given password
	if(!userExists(tx, username)) return addUsertoRoom(tx, username, password);
}

result addChattoRoom(pqxx::work tx, string room, string username, string msgId, string msg){
...
bool createRoom(string name, vector<string> usersToAdd){//create new table for chats and add users to room table
