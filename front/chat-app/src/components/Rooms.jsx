...
const Rooms = ({ currentUser, onEnterRoom }) => {
  ...
  // Mock data for rooms - in a real app, this would come from the backend
  useEffect(() => {
    // Simulate loading rooms from backend
    const mockRooms = [
      { id: 'room1', name: 'General Discussion', members: 4 },
      { id: 'room2', name: 'Tech Talk', members: 6 },
      { id: 'room3', name: 'Random Chat', members: 3 }
    ];
    setRooms(mockRooms);
  }, []);
  ...
  const handleJoinRoom = (e) => {
    ...
    // In a real app, this would send a request to the backend to join the room
    // For now, we'll just check if the room exists in our mock data
    const roomExists = rooms.some(room => room.id === roomIdToJoin);
    
    if (roomExists) {
      // Navigate to the room
      onEnterRoom(roomIdToJoin);
      setRoomIdToJoin('');
    ...
    <div className="flex-1 pt-32 px-4 bg-gray-100">
      <div className="max-w-screen-xl mx-auto">
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          {rooms.length === 0 ? (
            <p className="text-gray-600 text-center py-8">You haven't joined any rooms yet.</p>
          ) : (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {rooms.map(room => (
                <div key={room.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <p className="text-gray-600 text-sm mb-3">{room.members} members</p>
                  <button 
